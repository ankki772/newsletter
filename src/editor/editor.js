...
export default function EditorPage() {
  ...
  useEffect(() => {
    refreshRecords();
    getOriginalTemplate()
    .then((res) =>{ const r = localStorage.getItem('recentTemplate');
    console.log("000000000000000000000000000000",r)
      if(r){
        console.log("===========>>>>>>>>>>>>>>",r)
        setValue(r);
      }
        ...
        setValue(res.data.data.template_code);
      }
    })
  ...
  const logLocal = () => {
    // const templateNode =  document.getElementById('templateImage')
    localStorage.setItem("recentTemplate", editorRef.current.getContent());
  };
  console.log(value);

  const sendLetter = async (value, emails) => {
    await postLetter(value, emails)
      .then((res) => {
        toast("Email Sent Successfully");
      })
      .catch((err) => {
        toast("Failed Sending Email");
      });
  };
      ...
      <div style={{ display: "flex", margin: "3px", zIndex: 1 }}>
        ...
        >
          ...
          <button
            className="gradient-button gradient-button-4"
            onClick={() => {
              sendLetter(value, chipsvalue.chips);
            }}
        ...
        <Editor
          ...
          init={{
            selector: ".texteditor",
            plugins:
              "advlist lists table paste textcolor fullscreen colorpicker tabfocus link preview autolink image charmap image code",
